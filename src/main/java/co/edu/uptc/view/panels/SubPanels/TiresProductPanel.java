...
public class TiresProductPanel extends JPanel {
    ...
    private List<Tire> allProducts;
    private List<Tire> filteredProducts;
    private Tire selectedProduct;
    ...
    private JTextField sizeSearchField;
    private JTextField typeSearchField;
    ...
    public TiresProductPanel(ViewController controller, Presenter presenter) {
    ...
    private void loadDataFromPresenter() {
        this.allProducts = presenter.requestTireList();
    ...
    private JPanel createFilterPanel() {
        ...
        searchField = createStyledSearchField("Nombre de la llanta");
        ...
        lblSize.setFont(new Font("Segoe UI", Font.BOLD, 25));
        lblSize.setForeground(Color.BLACK);
        lblSize.setAlignmentX(Component.LEFT_ALIGNMENT);

        sizeSearchField = createStyledSearchField("Buscar por tamaño...");
        sizeSearchField.addKeyListener(new KeyAdapter() {
            @Override
            public void keyReleased(KeyEvent e) {
                applyFilters();
            }
        ...
        lblType.setFont(new Font("Segoe UI", Font.BOLD, 25));
        lblType.setForeground(Color.BLACK);
        lblType.setAlignmentX(Component.LEFT_ALIGNMENT);

        typeSearchField = createStyledSearchField("Buscar por tipo...");
        typeSearchField.addKeyListener(new KeyAdapter() {
            @Override
            public void keyReleased(KeyEvent e) {
                applyFilters();
            }
        ...
        panel.add(lblSize);
        panel.add(sizeSearchField);
        panel.add(Box.createVerticalStrut(15));

        panel.add(lblType);
        panel.add(typeSearchField);
        panel.add(Box.createVerticalStrut(15));
    ...
    private void applyFilters() {
        ...
        String sizeSearch = sizeSearchField.getText().trim().toLowerCase();
        String typeSearch = typeSearchField.getText().trim().toLowerCase();
        ...
        filteredProducts = allProducts.stream()
                ...
                .filter(p -> sizeSearch.isEmpty()
                        || (p.getSize() != null && p.getSize().toLowerCase().contains(sizeSearch)))
                .filter(p -> typeSearch.isEmpty()
                        || (p.getType() != null && p.getType().toLowerCase().contains(typeSearch)))
        ...
        if (rbMayorPrecio.isSelected()) {
            ...
            filteredProducts.sort(Comparator.comparingDouble(Tire::getPrice));
        }
    ...
    private JPanel createCard(Tire product) {
        ...
        String specs = product.getSize() + " / " + product.getType();
    ...
    private void updatePagination() {
        ...
        editBtn.addActionListener(e -> {
            ...
            EditCreateProductPanel editPanel = new EditCreateProductPanel(
                    ...
                    selectedProduct,
                    (updatedProduct) -> {
                        boolean success = presenter.saveTire((Tire) updatedProduct);
                        if (success) {
                            loadDataFromPresenter();
                            applyFilters();
                            ...
                            ConfirmDialog.showErrorDialog(SwingUtilities.getWindowAncestor(this),
                                    "Error al guardar la llanta.", "Error");
                        }
                    },
                    this, controller, presenter);
            showDialog(editPanel, "Editar Llanta");
        });
        ...
        deleteBtn.addActionListener(e -> {
            ...
            if (confirmed) {
                boolean success = presenter.requestTireDeletion(selectedProduct.getId());
                if (success) {
                    loadDataFromPresenter();
                    applyFilters();
        ...
        addBtn.addActionListener(e -> {
            Tire newTire = new Tire(null, null, "", 0, "", "", "");
            EditCreateProductPanel addPanel = new EditCreateProductPanel(
                    "Crear Llanta",
                    newTire,
                    (addedProduct) -> {
                        boolean success = presenter.saveTire((Tire) addedProduct);
                        if (success) {
                            loadDataFromPresenter();
                            applyFilters();
                            SuccessPopUp.showSuccessPopup((Frame) SwingUtilities.getWindowAncestor(this), "Éxito:",
                            ...
                            ConfirmDialog.showErrorDialog(SwingUtilities.getWindowAncestor(this),
                                    "Error al crear la llanta.", "Error");
                        }
                    },
                    this, controller, presenter);
            showDialog(addPanel, "Crear Llanta");
        });
