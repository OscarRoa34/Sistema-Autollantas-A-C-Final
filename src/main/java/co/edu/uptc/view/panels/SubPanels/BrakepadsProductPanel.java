...
public class BrakepadsProductPanel extends JPanel {
    ...
    private List<BrakePad> allProducts;
    private List<BrakePad> filteredProducts;
    private BrakePad selectedProduct;
    ...
    private JTextField vehicleSearchField;
    ...
    public BrakepadsProductPanel(ViewController controller, Presenter presenter) {
    ...
    private void loadDataFromPresenter() {
        this.allProducts = presenter.requestBrakePadList();
        if (this.allProducts == null) {
            this.allProducts = new ArrayList<>();
            System.err.println("BrakePadsProductPanel: El presenter devolvió una lista nula.");
        }
    ...
    private JPanel createFilterPanel() {
        ...
        searchField = createSearchField("Buscar por nombre");
        ...
        brandSearchField = createSearchField("Buscar por marca");
        brandSearchField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 30));
        ...
        lblMaterial.setFont(new Font("Segoe UI", Font.BOLD, 20));
        lblMaterial.setAlignmentX(Component.LEFT_ALIGNMENT);

        materialSearchField = createSearchField("Buscar por material");
        materialSearchField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 30));
        materialSearchField.addKeyListener(new KeyAdapter() {
            @Override
            public void keyReleased(KeyEvent e) {
                applyFilters();
            }
        ...
        lblVehiculo.setFont(new Font("Segoe UI", Font.BOLD, 20));
        lblVehiculo.setAlignmentX(Component.LEFT_ALIGNMENT);

        vehicleSearchField = createSearchField("Buscar por compatibilidad");
        vehicleSearchField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 30));
        vehicleSearchField.addKeyListener(new KeyAdapter() {
            @Override
            public void keyReleased(KeyEvent e) {
                applyFilters();
            }
        ...
        ActionListener priceListener = e -> applyFilters();
        rbMayorPrecio.addActionListener(priceListener);
        rbMenorPrecio.addActionListener(priceListener);
        ...
        panel.add(materialSearchField);
        panel.add(Box.createVerticalStrut(10));
        panel.add(lblVehiculo);
        panel.add(vehicleSearchField);
    ...
    private void applyFilters() {
        ...
        String materialSearch = materialSearchField.getText().trim().toLowerCase();
        String vehicleSearch = vehicleSearchField.getText().trim().toLowerCase();
        ...
        filteredProducts = allProducts.stream()
                ...
                .filter(p -> materialSearch.isEmpty()
                        || (p.getMaterialType() != null && p.getMaterialType().toLowerCase().contains(materialSearch)))
                .filter(p -> vehicleSearch.isEmpty() || (p.getVehicleCompatibility() != null
                        && p.getVehicleCompatibility().toLowerCase().contains(vehicleSearch)))
        ...
        if (rbMayorPrecio.isSelected()) {
            ...
            filteredProducts.sort(Comparator.comparingDouble(BrakePad::getPrice));
        }
    ...
    private JPanel createCard(BrakePad product) {
        ...
        infoPanel.add(new JLabel("Material: " + product.getMaterialType(), SwingConstants.CENTER) {
            {
                setFont(new Font("Segoe UI", Font.PLAIN, 12));
            }
        });

        infoPanel.add(new JLabel("Compatible: " + product.getVehicleCompatibility(), SwingConstants.CENTER) {
            {
                setFont(new Font("Segoe UI", Font.PLAIN, 12));
            }
        });
    ...
    private void updatePagination() {
        ...
        editBtn.addActionListener(e -> {
            ...
            EditCreateProductPanel editPanel = new EditCreateProductPanel(
                    "Editar Pastilla de Freno",
                    selectedProduct,
                    (updatedProduct) -> {
                        boolean success = presenter.saveBrakePad((BrakePad) updatedProduct);
                        if (success) {
                            loadDataFromPresenter();
                            applyFilters();
                            ...
                            ConfirmDialog.showErrorDialog(SwingUtilities.getWindowAncestor(this),
                                    "Error al guardar la pastilla.", "Error");
                        }
                    },
                    this, controller, presenter);
            showDialog(editPanel, "Editar Pastilla de Freno");
        });
        ...
        deleteBtn.addActionListener(e -> {
            ...
            if (confirmed) {
                boolean success = presenter.requestBrakePadDeletion(selectedProduct.getId());
                if (success) {
                    loadDataFromPresenter();
                    applyFilters();
        ...
        addBtn.addActionListener(e -> {
            BrakePad newBrakePad = new BrakePad(null, null, "", "", 0, "", "");
            EditCreateProductPanel addPanel = new EditCreateProductPanel(
                    "Crear Pastilla de Freno",
                    newBrakePad,
                    (addedProduct) -> {
                        boolean success = presenter.saveBrakePad((BrakePad) addedProduct);
                        if (success) {
                            loadDataFromPresenter();
                            applyFilters();
                            SuccessPopUp.showSuccessPopup((Frame) SwingUtilities.getWindowAncestor(this), "Éxito:",
                            ...
                            ConfirmDialog.showErrorDialog(SwingUtilities.getWindowAncestor(this),
                                    "Error al crear la pastilla.", "Error");
                        }
                    },
                    this, controller, presenter);
            showDialog(addPanel, "Crear Pastilla de Freno");
        });
